...
// 1. Parse .env file from possible locations
function parseEnv() {
  const possiblePaths = [
    path.join(__dirname, '.env'),
    path.join(__dirname, '../backend-go/.env'),
  ...
  if (!config.CLOUDFLARE_R2_ENDPOINT) {
    config.CLOUDFLARE_R2_ENDPOINT = 'https://d3740121cb0721130cd2faafaa6cfb6f.r2.cloudflarestorage.com';
  }
...
if (missing.length > 0) {
  console.error(`[Upload] Error: Missing R2 keys (${missing.join(', ')}). Please provide them in .env or environment.`);
  process.exit(1);
}
